...
SELECT id,city_name,office,responsible,stage,stage_date FROM public.hr_funnel_1022
where
...
SELECT city,type_position,source_attraction,date_vihoda_na_raboty,hr_manager_fio FROM public.hr_funnel_374
...
let setCity = (date, item) => {
  if (indcatorsCalc[date][item.city_name] === undefined) { indcatorsCalc[date][item.city_name] = { null: null }; }
};
let setResponsible = (date, item) => {
  if (indcatorsCalc[date][item.city_name][item.responsible] === undefined) {
    indcatorsCalc[date][item.city_name][[item.responsible]] = null;
  }
};
...
Object.keys(indcatorsCalc).map(date => {
  Object.keys(indcatorsCalc[date]).map(city => {
    ...
    Object.keys(indcatorsCalc[date][city]/* [responsible] */).map(responsible => {
      let retObj = {
        "Дата": date,
        "Город": city,
        // "Офис": office,
        "Ответственный": responsible == 'null' ? null : responsible,
      };
...
let calc = (serchObject) => {
  let { "Город": city, "Дата": date, "Ответственный": responsible } = serchObject;
  if (responsible == null) {
    debugger;
  }
  /*  */
  serchObject["Всего маршрутов"] = [...new Set(all_calls
    .filter(item => (item.stage_date == date))
    .filter(item => (item.city_name == city))
    .filter(item => (responsible == null ? true : item.responsible == responsible))
    .map(item => item.id))].length;
  /*  */
  serchObject["Целевые звонки"] =
    serchObject["Всего маршрутов"]
    /* serchObject["Нецелевые кандидаты"] = */
    - [...new Set(nonTargetedCandidates
      .filter(item => (item.stage_date == date))
      .filter(item => (item.city_name == city))
      .filter(item => (responsible == null ? true : item.responsible == responsible))
      .map(item => item.id))].length
    ...
    - [...new Set(notCalledContinuation
      .filter(item => (item.stage_date == date))
      .filter(item => (item.city_name == city))
      .filter(item => (responsible == null ? true : item.responsible == responsible))
      .map(item => item.id))].length;
  /*  */
  serchObject["Приглашения на собеседования"] = [...new Set(invite
    .filter(item => (item.stage_date == date))
    .filter(item => (item.city_name == city))
    .filter(item => (responsible == null ? true : item.responsible == responsible))
    .map(item => item.id))].length;
  /*  */
  serchObject["Собеседования"] = [...new Set(interview
    .filter(item => (item.stage_date == date))
    .filter(item => (item.city_name == city))
    .filter(item => (responsible == null ? true : item.responsible == responsible))
    .map(item => item.id))].length;
  /*  serchObject["Не пришли на собеседование"] = */
  -[...new Set(interviewSkip
    .filter(item => (item.stage_date == date))
    .filter(item => (item.city_name == city))
    .filter(item => (responsible == null ? true : item.responsible == responsible))
    .map(item => item.id))].length;
  /*  */
  serchObject["Найм"] = work
    .filter(item => item.date_vihoda_na_raboty == date)
    .filter(item => item.city == city)
    .filter(item => responsible == null ? true : item.hr_manager_fio == responsible)
    .map(item => item.id).length;
  /*  */
...
indcatorsCalcArr.map(item => {
  ...
  let { "Дата": date, "Город": city, "Ответственный": responsible, ...indicators } = item;
...
await session.clearRange("1s8NYDvQ0oI1IwaxHwcUsBcoqo-BNmYkVghgEnMDsuXk", "Лист3");
await session.setData("1s8NYDvQ0oI1IwaxHwcUsBcoqo-BNmYkVghgEnMDsuXk", "Лист3", calcResult.map(item => Object.values(item)));
